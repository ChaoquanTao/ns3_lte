...
int main (int argc, char *argv[])
{
	...
	Config::SetDefault ("ns3::LteUePhy::UeMeasurementsFilterPeriod", TimeValue(MilliSeconds (500)));

	Config::SetDefault ("ns3::LteSpectrumPhy::CtrlErrorModelEnabled", BooleanValue (true));
	Config::SetDefault ("ns3::LteSpectrumPhy::DataErrorModelEnabled", BooleanValue (true));
	Config::SetDefault ("ns3::LteHelper::UseIdealRrc", BooleanValue (true));
	Config::SetDefault ("ns3::LteHelper::UsePdschForCqiGeneration", BooleanValue (true));
//	Config::SetDefault("ns3::LteUePhy::RsrpSinrSamplePeriod",UintegerValue(65535));
//	Config::SetDefault ("ns3::LteUePhy::UeMeasurementsFilterPeriod", TimeValue(Seconds (1)));
	...
	uint32_t runId = 3;
	uint16_t numberOfRandomUes = 1600;
	uint16_t numberOfSkyUes  = 1600 ;
	double simTime = 0.5;
	...
	//use default handover algorithm
	...
	NodeContainer skyUeNodes ;
	enbNodes.Create (3);
	randomUeNodes.Create (numberOfRandomUes);
	skyUeNodes.Create(numberOfSkyUes);
	...
	enbPositionAlloc->Add (Vector (500.0, 500.0, 100.0));                       // eNB1
	enbPositionAlloc->Add (Vector (500+distance,  500.0, 100.0));                 // eNB2
	enbPositionAlloc->Add (Vector (500+distance * 0.5, 500+distance * 0.866, 100.0));   // eNB3
	...
	for(unsigned int i=0; i<numberOfRandomUes; i++){
...
/**	Ptr<ListPositionAllocator> skyPos = CreateObject<ListPositionAllocator>() ;
	for(unsigned int i=0; i<numberOfSkyUes; i++){
...
**/
	for(unsigned int i=0; i<numberOfSkyUes; i++){
	 ...
	 Ptr<RandomBoxPositionAllocator> randomUePositionAlloc = CreateObject<RandomBoxPositionAllocator> ();
	  Ptr<UniformRandomVariable> xVal = CreateObject<UniformRandomVariable> ();
	  xVal->SetAttribute ("Min", DoubleValue ((i%GRID_WIDTH)*COLUMN_INTERVAL));
	  xVal->SetAttribute ("Max", DoubleValue ( ((i%GRID_WIDTH)+1)*COLUMN_INTERVAL));
	  randomUePositionAlloc->SetAttribute ("X", PointerValue (xVal));
	  Ptr<UniformRandomVariable> yVal = CreateObject<UniformRandomVariable> ();
	  yVal->SetAttribute ("Min", DoubleValue ( (i/GRID_WIDTH)*ROW_INTERVAL));
	  yVal->SetAttribute ("Max", DoubleValue ( ((i/GRID_WIDTH)+1)*ROW_INTERVAL));
	  randomUePositionAlloc->SetAttribute ("Y", PointerValue (yVal));
	  Ptr<UniformRandomVariable> zVal = CreateObject<UniformRandomVariable> ();
	  zVal->SetAttribute ("Min", DoubleValue (50));
	  zVal->SetAttribute ("Max", DoubleValue (50));
	  randomUePositionAlloc->SetAttribute ("Z", PointerValue (zVal));
	  mobility.SetPositionAllocator (randomUePositionAlloc);

		std::cout<<"skyleft:"<<(i%GRID_WIDTH)*COLUMN_INTERVAL<<"skyright:"<<((i%GRID_WIDTH)+1)*COLUMN_INTERVAL<<"skytop:"<<(i/GRID_WIDTH)*ROW_INTERVAL<<"skybottom"<<((i/GRID_WIDTH)+1)*ROW_INTERVAL<<std::endl;
		mobility.SetMobilityModel ("ns3::GaussMarkovMobilityModel",
				"Bounds", BoxValue(Box((i%GRID_WIDTH)*COLUMN_INTERVAL, ((i%GRID_WIDTH)+1)*COLUMN_INTERVAL, (i/GRID_WIDTH)*ROW_INTERVAL, ((i/GRID_WIDTH)+1)*ROW_INTERVAL,50,55)),
		...
		mobility.Install(skyUeNodes.Get(i)) ;
	}
	// Create Devices and install them in the Nodes (eNB and UE)
	NetDeviceContainer enbDevs;
	NetDeviceContainer randomUeDevs;
	NetDeviceContainer skyUeDevs ;
	...
	skyUeDevs = lteHelper->InstallUeDevice(skyUeNodes);
	// Add X2 interface
	...
	ueNodes.Add(skyUeNodes);
	NetDeviceContainer ueDevs;
	ueDevs.Add (randomUeDevs);
	ueDevs.Add(skyUeDevs);
